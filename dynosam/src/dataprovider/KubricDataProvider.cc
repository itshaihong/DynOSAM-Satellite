...
class KubricAllLoader {
 public:
  ...
  cv::Mat getOpticalFlow(size_t idx) const {
    if (idx >= flow_file_names_.size()) {
        return cv::Mat::zeros(cv::Size(kWidth, kHeight), CV_32FC2);
    }
    cv::Mat flow;
 ...
 private:
  ...
  void assembleGroundTruth() {
    ...
    for (size_t i = 0; i < ground_truths_.size(); ++i) {
    LOG(INFO) << "[Kubric] GT[" << i << "] has "
              << ground_truths_[i].object_poses_.size()
              << " object poses";
}
  }
